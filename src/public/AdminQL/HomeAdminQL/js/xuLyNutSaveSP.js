    ...
    edit_form.addEventListener('submit', function(event) {
        ...
        .then(data => {
            if (data.success) {
                ...
                .then(() => {                    
                    window.location.href = '/page-qly-nuoc-hoa';
                });
            } else {                
                Swal.fire({
                    icon: 'error',
...
document.addEventListener('DOMContentLoaded', function() {
    const edit_form_daxoa = document.getElementById('edit-form-da-xoa');

    edit_form_daxoa.addEventListener('submit', function(event) {
        event.preventDefault(); // Ngăn chặn hành vi mặc định của form submit
        ...
        const New_Hot = document.getElementById('New_Hot').value;
        const SpMoi_SpNoiBat = document.getElementById('SpMoi_SpNoiBat').value;
        const DaXoa = document.getElementById('daxoa').value;
        const IdNam_Nu = document.getElementById('IdNam_Nu').value;
        ...
        // Gửi request fetch để tạo mới sản phẩm
        fetch(`/save-sp-nuochoa-da-xoa/${IdSP}`, {
            method: 'PUT',
            // headers: {
            //     'Content-Type': 'multipart/form-data',
            //     'Authorization': 'Bearer your_access_token_here'
        ...
        .then(data => {
            if (data.success) {
                ...
                .then(() => {                    
                    window.location.href = '/da-xoa-sp-nuochoa';
                });
            } else {                
                Swal.fire({
                    icon: 'error',
